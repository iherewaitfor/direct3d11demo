...
ID3D11Buffer*                       g_pCBTextDesc = NULL;
...
std::wstring VIDEO_FILE_NAME = L"guilin_640x360_yuv420.yuv";

FILE* infile = NULL;
unsigned char *buf = NULL; //

DemuxingDecoding* g_DemuxingDecoding = NULL;
VideoInfo         g_mp4videoInfo;
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    const std::wstring fileExt = L".mp4";
    size_t st = VIDEO_FILE_NAME.rfind(fileExt);
    if (st != std::wstring::npos) {
        size_t t = VIDEO_FILE_NAME.size();
        size_t t2 = st + fileExt.size();
        if (t == t2) {//是mp4文件
            g_DemuxingDecoding = new DemuxingDecoding(VIDEO_FILE_NAME);
            if (g_DemuxingDecoding->getVideoInfo(g_mp4videoInfo)) {
                g_videoWidth = g_mp4videoInfo.width;
                g_videoHeight = g_mp4videoInfo.height;
            }
        }
    }
...
void UpdateTexture() {
    ...
    if (g_DemuxingDecoding) {
        //mp4文件
        FrameData frameData;
        bool getVideoFrame = false;
        if (g_DemuxingDecoding->getNetxtFrame(frameData)) {
            getVideoFrame = true;
        }
        else {
            delete g_DemuxingDecoding;
            g_DemuxingDecoding = new DemuxingDecoding(VIDEO_FILE_NAME);
            if (g_DemuxingDecoding->getNetxtFrame(frameData)) {
                getVideoFrame = true;
            }
        ...
        if (getVideoFrame) {
            int offset = 0;
            memcpy(buf, frameData.dst_data[0], g_videoWidth * g_videoHeight);
            offset += g_videoWidth * g_videoHeight;
            memcpy(buf + offset, frameData.dst_data[1], g_videoWidth * g_videoHeight / 4);
            offset += g_videoWidth * g_videoHeight / 4;
            memcpy(buf + offset, frameData.dst_data[2], g_videoWidth * g_videoHeight / 4);
        }
    }
    else { //yuv文件
        if (infile == NULL) {
            if ((infile = _wfopen(VIDEO_FILE_NAME.c_str(), L"rb")) == NULL) {
