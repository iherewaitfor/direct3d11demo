...
RenderTextureClass                  g_renderTextureClass;
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"TutorialWindowClass", L"Direct3D 11 ShateTexture CreateShareTexture", WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
    ...
    g_renderTextureClass.Initialize(g_pd3dDevice, width, height);
...
{ //渲染到纹理
    ...
    //清除渲染到纹理的depth和stencil缓冲.
    g_renderTextureClass.ClearRenderTarget(g_pImmediateContext, g_pDepthStencilView, 0.0f, 0.0f, 1.0f, 1.0f);
...
void Render()
{
    g_renderTextureClass.m_pDXGIKeyedMutex->AcquireSync(0, INFINITE);
    RenderToTexture();
    // 设置渲染目标为窗口
    g_pImmediateContext->OMSetRenderTargets(1, &g_pRenderTargetView, g_pDepthStencilView);
    ...
    g_renderTextureClass.m_pDXGIKeyedMutex->ReleaseSync(0);
}
